#ifndef INFRA_MOER_MEASURE_HH
#define INFRA_MOER_MEASURE_HH
//---------------------------------------------------------------------------
#include <sys/types.h>
...
public:
  ...
  inline double 
  mCpuTime() const {
    return (double)totalCpuTime + ((double)totalCpuuTime / 1000000);
  }
  inline double
  mTotalTime() const {
    return totalSecs + (double)(totalUSecs)/1000000;
  }
  inline double
  mIoTime() const {
    return (mTotalTime() - mCpuTime());
  }
...
//     os << ioTime <<endl;
    ...
    if(outputTotal) {
      ...
      os << " cpu : " << cpuTime << " ("
	 << std::setw(5) << (cpuTime * 100) / outputTotal << "%)"
	 << " io : " << ioTime << " ("
	 << std::setw(5) << (ioTime * 100) / outputTotal << "%)";

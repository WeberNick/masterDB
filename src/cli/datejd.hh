...
class DateJd {
  ...
  public:
    ...
    inline DateJd& set_to_current_date() {
        _jd = jdOfToday();
        return (*this);
    }
  ...
  public:
    inline DateJd& add_days(uint x) {
        _jd += x;
        return (*this);
    }
    inline DateJd& subtract_days(uint x) {
        _jd -= x;
        return (*this);
    }
  ...
  public:
    inline DateJd& operator=(const DateJd& x) {
        _jd = x._jd;
        return (*this);
    }
...
inline std::ostream& operator<<(std::ostream& os, const DateJd& x) { return x.print(os, ':'); }

inline std::istream& operator>>(std::istream& is, DateJd& x) {
    int y, m, d;
    char s1, s2;
    is >> y >> s1 >> m >> s2 >> d;
    x.set(y, m, d);

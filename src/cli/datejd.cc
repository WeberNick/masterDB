...
bool DateJd::set(const char*& aStringPtr, bool aYearHigh, char sep) {
...
DateJd& DateJd::set(const uint y, const uint m, const uint d) {
    _jd = ymd2jd(y, m, d);
    return (*this);
}

void DateJd::set(const uint aYYYYMMDD) {
    uint x = aYYYYMMDD;
...
void DateJd::set(std::istream& is, const char aSep) {
    char lSep;
    uint y, m, d;
    is >> y;
    if (is.eof()) { return; }
    is >> lSep;
    if (is.eof()) { return; }
    if (aSep != lSep) { std::cerr << "Warning: wrong field separator in date: `" << lSep << "'." << std::endl; }
    is >> m;
    if (is.eof()) { return; }
    is >> lSep;
    if (is.eof()) { return; }
...
void DateJd::set(const uint aJD, bool) { _jd = aJD; }

DateJd::uint DateJd::ymd2jd(const uint ay, const uint am, const uint ad) {
    /* tuts
        double Y = (double) y;
        double M = (double) m;
        double D = (double) d;
...
void DateJd::jd2ymd(const uint jd, uint& y, uint& m, uint& d) {
...
std::ostream& DateJd::print(std::ostream& os, char sep) const {
    uint y = 0;
    uint m = 0;
    uint d = 0;
    get(y, m, d);
    os << y << sep << (m < 10 ? "0" : "") << m << sep << (d < 10 ? "0" : "") << d;
    return os;
}

std::ostream& DateJd::printXml(std::ostream& os) const {
    uint y = 0;
    uint m = 0;
    uint d = 0;
    get(y, m, d);
...
DateJd::uint DateJd::month() const {
    uint y, m, d;
    jd2ymd(_jd, y, m, d);
    return m;
}

DateJd::uint DateJd::day() const {
    uint y, m, d;
    jd2ymd(_jd, y, m, d);
    return d;
}

DateJd::uint DateJd::day_of_year() const { return (_jd - DateJd(year(), 1, 1).jd() + 1); }

DateJd::uint DateJd::dayOfTheYear() const { return (_jd - DateJd(year(), 1, 1).jd()); }

DateJd::Month DateJd::month_of_year() const { return (Month)(month() - 1); }

DateJd::Weekday DateJd::day_of_week() const {
    DateJd someSunday(1997, 10, 12);
    return (Weekday)((someSunday <= (*this)) ? (subtract_date(someSunday) % 7) : (someSunday.subtract_date(*this) % 7));
}
...
bool DateJd::is_leap_year() const {
    uint y = year();
    uint c = y % 100;
    uint r = y % 900;
...
int DateJd::getMonthNo(const char* aMonth) {
    int lRes = -1;
    if (0 == strcmp(aMonth, "January")) {
        lRes = 0;
    } else if (0 == strcmp(aMonth, "February")) {
        lRes = 1;
    } else if (0 == strcmp(aMonth, "March")) {
        lRes = 2;
    } else if (0 == strcmp(aMonth, "April")) {
        lRes = 3;
    } else if (0 == strcmp(aMonth, "May")) {
        lRes = 4;
    } else if (0 == strcmp(aMonth, "June")) {
        lRes = 5;
    } else if (0 == strcmp(aMonth, "July")) {
        lRes = 6;
    } else if (0 == strcmp(aMonth, "August")) {
        lRes = 7;
    } else if (0 == strcmp(aMonth, "September")) {
        lRes = 8;
    } else if (0 == strcmp(aMonth, "October")) {
        lRes = 9;
    } else if (0 == strcmp(aMonth, "November")) {
        lRes = 10;
    } else if (0 == strcmp(aMonth, "December")) {
        lRes = 11;
    } else {
        lRes = -1;
    }

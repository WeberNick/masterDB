...
int main(const int argc, const char *argv[]) {
    ...
    ***************************************************************************************************/
    ...
    PartitionFile *lPartFile =
        lPartMngr.createPartitionFileInstance(argv[1], c_PartitionSizeInPages(), c_PageSize(), c_GrowthIndicator());
    ...
    if (lPartFile->createPartition() != 0) // This could be commented out
        std::cout << "\033[0;31m> Something went wrong while creating the partition...\033[0m"
                  << std::endl; // will be printed as long as reservePage in fsip is not implemented
    /* Add further tests...
    ...
    */
     ...
     ***************************************************************************************************/
    if (lPartMngr.getPartition(lPartFile->getID())->open() == -1) {
    ...
    if (lPartMngr.getPartition(lPartFile->getID())->close() == -1) {
        std::cout << "\033[0;31m> Something went wrong while closing the partition...\033[0m" << std::endl;
    }

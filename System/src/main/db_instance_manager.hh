...
{
	SegmentFSM_SP lSegments(_masterPartition);
...
{
	SegmentFSM_SP lMasterSeg(_masterPartition);
    ...
    int lCapazSoll = _segmentTuples.size()/sizeof(T_TupleType);
    ...
    for (uint i = 0; i < lMasterSeg.getNoPages(); ++i)
    {
        ...
        while(true){
            ...
            *((T_TupleType*) lPos ) =  _segmentTuples.at(lSegCounter);
            ++lSegCounter;
        }   
    }
	delete[] lPage;

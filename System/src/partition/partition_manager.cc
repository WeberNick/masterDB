...
PartitionBase* PartitionManager::getPartition(const uint8_t aID)
{
    ...
    if (_partitions.find(aID)==_partitions.end()) {
        ...
        control_block_t lcontrolBlock = control_block_t {"a","b","c",10,0};
        switch(lTuple._pType){
            case 1://PartitionFile
            s = new PartitionFile(lTuple,lcontrolBlock);
            break;
            case 2: //PartitionRaw
            s = new PartitionRaw(lTuple,lcontrolBlock);
            break;
...
PartitionBase* PartitionManager::createMasterPartition(std::string aPath, uint aGrowthIndicator, control_block_t& aControlBlock,part_t& aMasterTuple){
    if(_installed!=0){
        printErr("Master Partition already created");
        return nullptr;
    }
     int pType = 1;
    PartitionFile* lPartition = new PartitionFile(aPath, aControlBlock.mstrPart(), aGrowthIndicator, _counterPartitionID++, aControlBlock);
    _partitions[lPartition->getID()] = lPartition;

    part_t lp = {lPartition->getID(),aControlBlock._masterPartition,aPath,pType,aGrowthIndicator};
    aMasterTuple=lp;
    _installed++;
    return (PartitionFile*)_partitions.at(lPartition->getID());
}

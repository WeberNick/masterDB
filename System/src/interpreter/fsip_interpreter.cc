...
const int FSIPInterpreter::getNewBlock(byte* aPP)
{
	...
	for(uint32_t j = lReturnValue; j<=(lInterp.getBlockSize() - (sizeof(FSIP_header_t) + lInterp.getHeaderSize())) - 1; ++j){ //hier wären kommentare schön, hab es mal formatiert damit es leserlicher ist.
